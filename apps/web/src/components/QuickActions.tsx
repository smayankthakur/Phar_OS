...
export function QuickActions({ clientDemoMode = false }: { clientDemoMode?: boolean }) {
  const router = useRouter();
  const [simulationType, setSimulationType] = useState<SimulationType>("COMPETITOR_PRICE_DROP");
  ...
  return (
    <div className="detail-grid">
      <div className="quick-actions">
        ...
        <select value={simulationType} onChange={(event) => setSimulationType(event.target.value as SimulationType)}>
        ...
        <Button variant="secondary" loading={pendingSim} loadingText="Running..." onClick={runSimulation}>
          Run Simulation
        </Button>
